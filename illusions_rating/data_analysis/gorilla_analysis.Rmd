...
# Analysis of motion illusions data
...
## Preprocessing. Skip if you have downloaded the demographics.csv file

```{r}
demographics_df = read.csv("../prolific_demographics/prolific_demographic_export_67e4d8aad6876bff90464913.csv")

# remove irrelevant info
unique(demographics_df$Status)
demographics_df$Custom.study.tncs.accepted.at = NULL
demographics_df$Completion.code = NULL

# Remove excluded people
a = demographics_df$Status == "APPROVED"
a = demographics_df[a,]
demographics_df = a

# convert numbers
demographics_df$Age = as.numeric(as.character(demographics_df$Age))

# save
write.csv2(demographics_df, "../gorilla_data/2025/demographics.csv", row.names = FALSE)
```
...
List files tasks for later

```{r}
for (file_id in c(1:length(files_list))){
  file_path = files_list[file_id]
  df = read.csv(file_path, stringsAsFactors=F)
  print(file_id)
  print(unique(df$Task.Name))
}
```


Remove participants who did not answer "cat" on attention test, participants who failed on instructional manipulation and participants who revoked consent

```{r}
file_path = files_list[15]
df = read.csv(file_path, stringsAsFactors=F)
sub = subset(df, df$Response == "cat2.jpg")
rejected_participants = subset(df, !(df$Participant.External.Session.ID %in% sub$Participant.External.Session.ID))
rejected_ids = unique(rejected_participants$Participant.External.Session.ID)
...
df = read.csv(file_path, stringsAsFactors=F)
unique(df$Task.Name)
unique(df$Multiple.Choice.object.4.Response)
sub = subset(df, df$Multiple.Choice.object.4.Response == "Purple")
rejected_participants = subset(df, df$Response != "Purple")
rbind(rejected_ids, unique(rejected_participants$Participant.External.Session.ID))
results = subset(results, results$participant_id %in% sub$Participant.External.Session.ID)

# cross reference with demographics data approved list
results = subset(results, results$participant_id %in% demographics_df$Submission.id)
```

## Subset corresponding demographics

```{r}
demographics_df_sub = subset(demographics_df, demographics_df$Submission.id %in% results$participant_id)
mean(demographics_df_sub$Age)
sd(demographics_df_sub$Age)
min(demographics_df_sub$Age)
max(demographics_df_sub$Age)
```
...
# save as pdf
pdf(file = paste0(figures_path, "strength.pdf"),   # The directory you want to save the file in
...
How many say the snakes do not rotate?

```{r}
id = 9 # snakes
sub = subset(results, results$illusion_id == id)
n = length(sub$participant_id)
hist(sub$normalized)
length(subset(sub, normalized <= 0.33)$participant_id)/n

# How many said the control rotated?
...
## Median of normalized values, with errors
...
```{r}
# Welch Two Sample t-test: https://www.datacamp.com/tutorial/t-tests-r-tutorial
# Check if each illusion data is different from control
control_image_results = subset(results, results$illusion_id == 7)

wt_test = data.frame()

for (i in c(1:length(ids$illusion_id))){
  sub = subset(results, results$illusion_id == ids$illusion_id[i])
  a = t.test(control_image_results$strength, sub$strength)
  wt_test = rbind(wt_test, c(ids$illusion_name[i], ids$illusion_id[i], a$p.value))
}

colnames(wt_test) = c("illusion_name", "illusion_id", "p_value")
wt_test
```
...
# save as pdf
pdf(file = paste0(figures_path, "orientation.pdf"),   # The directory you want to save the file in
    width = 7, # The width of the plot in inches
    height = 5) #
...
---- Figures for demographic data ----


## Make figures

```{r}
save_as_pdf = function(my_plot, name){
  pdf(file = paste0(figures_path, name),   # The directory you want to save the file in
...
# Age
# save as pdf
pdf(file = paste0(figures_path, "demographics-age.pdf"),   # The directory you want to save the file in
    width = 8, # The width of the plot in inches
    height = 5) # 

hist(demographics_df$Age, xlab = "Age", xlim = c(0,100), main = "")
